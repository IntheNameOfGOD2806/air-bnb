...
const Title = ({isTour, isVehicle}) => {
    const { title, setTitle } = useAppstore();
    return (
        <div className="flex flex-col gap-5 items-center justify-center text-black h-full ">
            <div className="flex flex-col gap-2">
                <h2 className="font-semibold text-4xl text-center">{isTour ? "Tiêu đề cuả tour của bạn" : isVehicle ? "Tiêu đề cuả xe của bạn" : "Tiêu đề cuả điểm lưu trú"}</h2>
                <p className="text-center">Một tiêu đề hấp dẫn có thể thu hút khách hàng và giúp họ hiểu rõ hơn về {isTour ? "tour" : isVehicle ? "xe" : "điểm lưu trú"} của bạn.</p>
            </div>
